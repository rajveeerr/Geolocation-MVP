  ...
  return (
    <div className="group w-full cursor-pointer">
      <div className="relative aspect-square overflow-hidden rounded-xl shadow-sm transition-shadow duration-200 hover:shadow-md sm:rounded-2xl">
        ...
        <button className="absolute right-2 top-2 flex h-7 w-7 items-center justify-center rounded-full bg-black/20 text-white backdrop-blur-sm transition-all hover:scale-110 hover:bg-black/40 sm:right-3 sm:top-3 sm:h-8 sm:w-8">
      ...
      <div className="mt-2 sm:mt-3">
        <div className="flex items-start justify-between gap-2">
          ...
          <div className="flex flex-shrink-0 items-center gap-0.5 sm:gap-1">
            <Star className="h-3.5 w-3.5 fill-current text-neutral-800 sm:h-4 sm:w-4" />
            <span className="text-xs font-medium sm:text-sm">
              {deal.rating.toFixed(1)}
            </span>
          </div>
        </div>
        <p className="mt-0.5 truncate text-xs text-neutral-500 sm:mt-1 sm:text-sm">
          {deal.location}
        </p>
