...
export const HowItWorksSection = () => {
  return (
    <section className="bg-[#FAF9F7] py-20 md:py-28 px-6 lg:px-8">
      <div className="max-w-5xl mx-auto text-center">
        {/* Section Label */}
        <p className="text-[12px] font-bold uppercase tracking-[0.15em] text-[#8B1A1A] mb-3">
          The Game
        </p>
            ...
            {steps.map((s, i) => (
              <div key={s.step} className="relative flex flex-col items-center text-center">
                ...
                <div className="relative mb-6 z-10">
                  {/* Outer ring */}
                  <div className="w-[72px] h-[72px] rounded-full border-2 border-[#8B1A1A]/30 flex items-center justify-center bg-[#FAF9F7]">
                    {/* Inner icon */}
                    <s.icon className="h-7 w-7 text-[#8B1A1A]" strokeWidth={1.8} />
                  </div>
                  {/* Step number badge */}
                  <span className="absolute -bottom-2 left-1/2 -translate-x-1/2 bg-[#8B1A1A] text-white text-[10px] font-bold rounded-full px-2 py-0.5 min-w-[46px] h-[20px] flex items-center justify-center shadow-sm whitespace-nowrap">
                    Step {s.step}
                  </span>
                </div>
