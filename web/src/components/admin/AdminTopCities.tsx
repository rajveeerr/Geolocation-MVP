...
import { TrendingUp, TrendingDown, Minus, MapPin, DollarSign, ChevronDown, ChevronUp } from 'lucide-react';
...
export const AdminTopCities: React.FC<AdminTopCitiesProps> = ({
    ...
    return (
      <div className="rounded-2xl border border-red-100 bg-white p-6 shadow-sm">
        <div className="flex items-center justify-center h-32">
          <span className="text-sm text-red-400">Failed to load top cities data</span>
        </div>
      </div>
    );
  }
  ...
  const { cities } = data;
  const displayCities = isExpanded ? cities : cities.slice(0, 5);
  const hasMore = cities.length > 5;
  ...
  return (
    <div className="rounded-2xl border border-neutral-200/60 bg-white p-6 shadow-sm">
      <div className="flex items-center gap-2.5 mb-5">
        ...
        <h3 className="text-base font-bold font-heading text-neutral-900">Top Cities</h3>
        ...
        {displayCities.map((city, index) => (
          <div key={city.id}
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h4 className="text-sm font-semibold text-neutral-800">{city.name}</h4>
                <p className="text-[12px] text-neutral-400">{city.state}</p>
              </div>
            </div>
            <div className="text-right">
              ...
              <div className={`flex items-center gap-0.5 text-[12px] justify-end ${getTrendColor(city.trend)}`}>
                {getTrendIcon(city.trend)}
                {city.change > 0 ? '+' : ''}{city.change.toFixed(1)}%
              </div>
            </div>
          </div>
        ))}
        ...
        <div className="mt-3 pt-3 border-t border-neutral-100">
          <Button variant="ghost" onClick={() => setIsExpanded(!isExpanded)}
            ...
            {isExpanded
              ...
              : <><ChevronDown className="h-3.5 w-3.5" /> Show All ({cities.length})</>}
          </Button>
        </div>
      )}
      ...
      {cities.length === 0 && (
        <div className="text-center py-10">
          ...
          <p className="text-sm text-neutral-400">No city data available</p>
        </div>
      )}
    </div>
  );
