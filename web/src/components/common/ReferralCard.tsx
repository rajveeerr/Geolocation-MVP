...
export const ReferralCard = ({ code, count }: ReferralCardProps) => {
  ...
  const handleShare = () => {
    if (navigator.share) {
      ...
      toast({ title: 'Link Copied!', description: 'Sharing is not supported on your browser, but the invite link is on your clipboard.' });
    }
  };
  ...
  <div className="bg-white rounded-2xl border border-neutral-200 shadow-sm p-6 sm:p-8 space-y-6 w-full">
    ...
    <div className="text-center">
      ...
      <p className="mt-4 text-sm font-semibold text-neutral-500 uppercase tracking-wider">Your Unique Referral Code</p>
      <div className="mt-2 flex justify-center items-center gap-2">
        <div className="text-4xl font-bold text-brand-primary-700 tracking-widest bg-neutral-100 border-2 border-dashed border-neutral-200 rounded-xl px-6 py-3">
          {code}
        </div>
        <Button onClick={handleCopyCode} variant="secondary" size="md" className="h-16 w-16 rounded-xl border-neutral-300">
          {copied ? <Check className="h-6 w-6 text-status-live" /> : <Copy className="h-6 w-6" />}
        </Button>
      </div>
    </div>
    ...
    <div className="border-t border-neutral-200 pt-6">
      <div className="flex items-center justify-center gap-4">
        ...
        <div className="text-left">
          ...
          <p className="text-sm text-neutral-500 -mt-1">Friends have joined!</p>
        </div>
      </div>
    </div>
  </div>
