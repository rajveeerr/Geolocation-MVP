  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-2">
        ...
        <h3 className="text-lg font-semibold text-foreground">Featured Media</h3>
      ...
      {images.length > 0 ? (
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
          {images.map((imageUrl, index) => (
            ...
            >
              ...
              <div className="absolute bottom-2 left-2 rounded-full bg-foreground/50 px-2 py-1 text-xs text-background">
                {index + 1}
              </div>
            </motion.div>
          ))}
          {images.length < maxImages && (
            <button
              ...
              className="flex aspect-square items-center justify-center rounded-lg border-2 border-dashed border-border text-muted-foreground transition-colors hover:border-brand-primary-300 hover:bg-brand-primary-25 hover:text-brand-primary-700"
        ...
        <button
          type="button"
          onClick={() => setUploadModalOpen(true)}
          className="flex w-full flex-col items-center justify-center rounded-xl border-2 border-dashed border-border py-12 transition-colors hover:border-border hover:bg-muted"
        >
          <div className="flex h-16 w-16 items-center justify-center rounded-full bg-muted">
            <ImageIcon className="h-8 w-8 text-muted-foreground" />
          </div>
          <p className="mt-4 text-base font-semibold text-foreground">Add photos</p>
          <p className="mt-1 text-sm text-muted-foreground">Drag and drop or browse. PNG, JPG, GIF up to 5MB.</p>
        </button>
      )}
      ...
      <div className="rounded-lg border border-border bg-muted p-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h4 className="font-medium text-foreground">Image Tips</h4>
