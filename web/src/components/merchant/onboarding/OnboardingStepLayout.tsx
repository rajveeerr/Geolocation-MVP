...
function WizardStepper({ current, total }: WizardStep) {
  ...
  return (
    <div
      className="mb-6 rounded-2xl border border-border/80 bg-card/90 dark:bg-card p-4 shadow-[0_10px_28px_rgba(15,23,42,0.06)] backdrop-blur-sm ring-1 ring-neutral-900/[0.03]"
      role="navigation"
...
export const OnboardingStepLayout = ({
  ...
  return (
    <div className="relative -mx-4 -my-5 flex min-h-[calc(100vh-5rem)] flex-col overflow-hidden bg-background sm:-mx-6 sm:-my-6 lg:-mx-8 lg:-my-8">
      <div className="pointer-events-none absolute inset-0">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_left,rgba(232,2,3,0.12),transparent_44%),radial-gradient(circle_at_top_right,rgba(15,23,42,0.06),transparent_40%),linear-gradient(180deg,#fbfbfc_0%,#f7f7f9_52%,#f5f5f7_100%)] dark:bg-none" />
        <div className="absolute left-[-10rem] top-24 h-72 w-72 rounded-full bg-[hsl(var(--brand-100))]/45 blur-3xl" />
        <div className="absolute right-[-7rem] top-8 h-80 w-80 rounded-full bg-accent/55 blur-3xl" />
      </div>
      ...
      >
        <div className={contentClassName ?? 'w-full max-w-3xl'}>
          ...
          >
            <h1 className="max-w-[16ch] text-balance font-serif text-[2rem] font-semibold leading-[0.95] tracking-tight text-foreground sm:text-[2.3rem] lg:max-w-none lg:text-[2.55rem]">
              {title}
            </h1>
            {subtitle ? (
              <p className="mt-2 max-w-3xl text-[14px] leading-6 text-muted-foreground sm:text-[15px] lg:text-base">
                {subtitle}
              </p>
            ) : null}
          </motion.div>
      ...
      <footer className="fixed bottom-0 left-0 right-0 z-30 border-t border-border/80 bg-card/92 dark:bg-card backdrop-blur-xl shadow-[0_-14px_30px_rgba(15,23,42,0.10)] lg:left-[320px] lg:w-[calc(100%-320px)]">
        <div className="mx-auto flex min-h-[5.5rem] w-full max-w-screen-xl items-center justify-between gap-4 px-4 py-4 sm:min-h-[6rem] sm:px-6 lg:px-8">
          <Button
            variant="secondary"
            onClick={onBack}
            className="h-11 rounded-2xl border-border bg-card px-6 text-foreground shadow-none hover:border-border hover:bg-muted"
            disabled={isLoading}
          >
            Back
          </Button>
