...
export const StoreFeaturesStep = ({ data, onUpdate }: StoreFeaturesStepProps) => {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h2 className="text-2xl font-bold text-foreground">Does this location have any of these?</h2>
        <p className="mt-2 text-muted-foreground">Select what customers can expect. You can change this later.</p>
      </div>
      <div className="space-y-3">
        {FEATURES.map((f) => {
          ...
          return (
            <button key={f.id} type="button" onClick={() => toggle(f.id)} className={cn('flex w-full items-center gap-4 rounded-xl border-2 p-4 text-left transition-all', checked ? 'border-brand-primary-500 bg-brand-primary-50' : 'border-border bg-card hover:border-border')}>
              <div className={cn('flex h-10 w-10 shrink-0 items-center justify-center rounded-full', checked ? 'bg-brand-primary-500 text-white' : 'bg-muted text-muted-foreground')}>
              ...
              <div className={cn('h-5 w-5 shrink-0 rounded border-2', checked ? 'border-brand-primary-500 bg-brand-primary-500' : 'border-border')}>
