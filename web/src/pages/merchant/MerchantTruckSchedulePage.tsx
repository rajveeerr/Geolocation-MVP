...
function MerchantTruckScheduleContent() {
  ...
  if (!store) {
    return (
      <div className="mx-auto max-w-screen-md px-4 py-8 text-center">
        ...
        <p className="mt-2 text-sm text-muted-foreground">This store doesn't belong to your account, or it was removed.</p>
  ...
  return (
    <div className="mx-auto max-w-screen-md space-y-6 px-4 py-3 sm:px-1 sm:py-4">
      ...
      {store.isFoodTruck && (current || next) && (
        <div className={panelClass}>
          <div className="grid divide-y divide-neutral-200 sm:grid-cols-2 sm:divide-x sm:divide-y-0">
            <div className="p-5">
              <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Live now</p>
              {current ? (
                <>
                ...
                <p className="mt-2 text-sm text-muted-foreground">Not at a stop right now.</p>
              )}
            </div>
            <div className="p-5">
              <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Next stop</p>
              {next ? (
                <>
                  <p className="mt-2 flex items-start gap-2 text-sm font-medium text-foreground">
                ...
                <p className="mt-2 text-sm text-muted-foreground">No upcoming stops.</p>
              )}
            </div>
          </div>
        </div>
      ...
      {!store.isFoodTruck && (
        <div className={`${panelClass} border-amber-200 dark:border-amber-900/50 bg-amber-50 dark:bg-amber-950/30 p-5 text-sm text-amber-800 dark:text-amber-300`}>
