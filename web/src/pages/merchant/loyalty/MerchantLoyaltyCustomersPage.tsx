  ...
  return (
    ...
    >
      <div className="mb-4 flex flex-wrap items-center gap-2 rounded-xl border border-border bg-card p-3">
        ...
        <span className="ml-auto text-sm text-muted-foreground">Total customers: {total}</span>
      </div>

      {isLoading ? (
        <div className="h-24 animate-pulse rounded-xl bg-muted" />
      ) : !data || data.customers.length === 0 ? (
        <div className="rounded-xl border border-border bg-muted p-4 text-muted-foreground">No customers found.</div>
      ) : (
        <div className="overflow-hidden rounded-xl border border-border bg-card">
          <table className="min-w-full divide-y divide-neutral-200">
            <thead className="bg-muted">
              <tr>
                <th className="px-4 py-2 text-left text-sm font-semibold text-foreground">Customer</th>
                <th className="px-4 py-2 text-left text-sm font-semibold text-foreground">Balance</th>
                <th className="px-4 py-2 text-left text-sm font-semibold text-foreground">Lifetime</th>
            ...
            <tbody className="divide-y divide-neutral-200 bg-card">
              {data.customers.map((c) => (
                <Fragment key={c.userId}>
                  <tr>
                    <td className="px-4 py-2">{c.userName} <div className="text-xs text-muted-foreground">{c.userEmail}</div></td>
                    <td className="px-4 py-2 font-semibold">{c.currentBalance} pts</td>
                    <td className="px-4 py-2 text-sm text-muted-foreground">Earned {c.lifetimeEarned} · Redeemed {c.lifetimeRedeemed}</td>
                        ...
                        <div className="rounded-lg border border-dashed border-border bg-muted p-3">
                          <div className="grid gap-2 sm:grid-cols-4">
                            <input
                              ...
                              placeholder="Points"
                            />
                            <select className="rounded-md border border-border p-2 text-sm" value={adjustType} onChange={(e) => setAdjustType(e.target.value as 'BONUS' | 'ADJUSTED' | 'REFUNDED')}>
                              <option value="BONUS">Bonus</option>
                              <option value="ADJUSTED">Adjusted</option>
                              <option value="REFUNDED">Refunded</option>
                            </select>
                            <input
                              className="rounded-md border border-border p-2 text-sm sm:col-span-2"
                              value={reason}
                          ...
                          <div className="mt-2 flex justify-end gap-2">
                            <button type="button" className="rounded-md border border-border px-3 py-1.5 text-sm" onClick={() => setAdjustingUserId(null)}>
                              Cancel
                            </button>
                            <button
                              type="button"
