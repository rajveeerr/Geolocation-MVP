...
function StatCard({ label, value }: { label: string; value: string | number }) {
  return (
    <div className={cn(panelClass, 'p-4')}>
      ...
      <div className="mt-1 text-[1.6rem] font-semibold tracking-tight text-foreground">{value}</div>
    </div>
  );
}
      ...
      <div className="flex gap-4 p-4">
        <div className="flex h-20 w-20 shrink-0 items-center justify-center overflow-hidden rounded-xl border border-border bg-muted">
          {item.imageUrl ? (
            ...
            <ChefHat className="h-7 w-7 text-muted-foreground" aria-hidden />
          )}
        </div>

        <div className="min-w-0 flex-1 space-y-2">
          <div className="flex flex-wrap items-center gap-2">
            <h3 className="truncate text-[1rem] font-semibold text-foreground">{item.name}</h3>
            {item.isPopular && (
              <span className="inline-flex items-center rounded-full bg-amber-100 dark:bg-amber-950/40 px-2 py-0.5 text-[10px] font-semibold text-amber-800 dark:text-amber-300 ring-1 ring-inset ring-amber-600/20">
                Most ordered
              </span>
            )}
            {!item.isActive && (
    ...
    <div className="mx-auto max-w-screen-xl space-y-6 px-4 py-3 sm:px-1 sm:py-4">
      <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h1 className="mt-2 text-[1.9rem] font-semibold tracking-tight text-foreground">Catering menu</h1>
      ...
      {items.length === 0 ? (
        <div className={cn(panelClass, 'border-dashed py-16 text-center')}>
          <ChefHat className="mx-auto mb-3 h-10 w-10 text-muted-foreground" aria-hidden />
          <h3 className="text-[1.4rem] font-semibold tracking-tight text-foreground">No catering items yet</h3>
          ...
          <div className={cn(panelClass, 'flex flex-wrap items-center gap-2 overflow-x-auto p-3')}>
            ...
            <button
              ...
              className={cn(
                ...
                activeCategory === 'ALL' ? 'bg-foreground text-background' : 'bg-muted text-muted-foreground hover:bg-accent',
              )}
            >
              All
              <span
                className={cn(
                  ...
                  activeCategory === 'ALL' ? 'bg-card/20 dark:bg-card' : 'bg-accent',
                )}
              >
                {items.length}
              </span>
            ...
            {categories.map((cat) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    'whitespace-nowrap rounded-full px-3 py-1.5 text-[13px] font-medium transition',
                    activeCategory === cat ? 'bg-foreground text-background' : 'bg-muted text-muted-foreground hover:bg-accent',
                  )}
                >
                  {cat}
                  <span
                    className={cn(
                      'ml-1.5 rounded-full px-1.5 py-0.5 text-xs',
                      activeCategory === cat ? 'bg-card/20 dark:bg-card' : 'bg-accent',
                    )}
                  >
                    {count}
                  </span>
            ...
            <label className="ml-auto inline-flex items-center gap-2 px-2 text-xs text-muted-foreground">
              <input
                type="checkbox"
