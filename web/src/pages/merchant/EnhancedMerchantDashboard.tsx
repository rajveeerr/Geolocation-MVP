  ...
  return (
    <div className="min-h-screen bg-muted py-8">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-foreground mb-2">Merchant Dashboard</h1>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div className="p-4 rounded-lg bg-muted">
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-sm text-muted-foreground">Deal Saves</span>
                  ...
                  <CardContent>
                    <div className="space-y-4">
                      <div className="flex items-center justify-between">
                        <span className="text-sm text-muted-foreground">Deal Views</span>
                      ...
                      <div className="flex items-center justify-between">
                        <span className="text-sm text-muted-foreground">Kickback Events</span>
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="space-y-4">
                      ...
                      <div className="flex items-center justify-between">
                        <span className="text-sm text-muted-foreground">Kickback Rate</span>
                        ...
                        <div key={deal.id} className="p-4 rounded-lg border bg-card">
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                            ...
                            <div className="text-center">
                              ...
                              <p className="text-sm text-muted-foreground">Kickbacks</p>
                            </div>
                            <div className="text-center">
                          ...
                          <div className="mt-4 pt-4 border-t">
                            <div className="grid grid-cols-2 gap-4">
                              <div>
                                <p className="text-sm text-muted-foreground">Save to Check-in Rate</p>
                                <p className="font-semibold">
                                  {(deal.performance.conversionRates.saveToCheckIn * 100).toFixed(1)}%
                                </p>
                              </div>
                              <div>
                                <p className="text-sm text-muted-foreground">Check-in to Kickback Rate</p>
                                <p className="font-semibold">
                    ...
                    <CardContent>
                      <div className="grid grid-cols-3 gap-4">
                        ...
                        <div className="text-center p-4 rounded-lg bg-yellow-50 dark:bg-yellow-950/30">
                          ...
                          <p className="text-sm text-muted-foreground">Medium Activity</p>
                        </div>
                    ...
                    <CardContent>
                      <div className="space-y-3">
                        {customerInsights.customerValue.topCustomers.slice(0, 10).map((customer, index) => (
                          <div key={customer.user.id} className="flex items-center justify-between p-3 rounded-lg bg-muted">
                            <div className="flex items-center gap-3">
                              <span className="text-sm font-bold text-muted-foreground">#{index + 1}</span>
                              <div>
                                <h4 className="font-semibold">{customer.user.name}</h4>
                                <p className="text-sm text-muted-foreground">{formatCount(customer.kickbackEvents)} kickbacks</p>
                              </div>
                            </div>
                            <div className="text-right">
                    ...
                    <Card>
                      <CardContent className="p-4">
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            <p className="text-sm text-muted-foreground">Kickback Paid</p>
                    ...
                    <CardContent>
                      <div className="space-y-3">
                        {revenueAnalytics.revenueByCategory.map((category) => (
                          <div key={category.category.name} className="flex items-center justify-between p-3 rounded-lg bg-muted">
                            <div className="flex items-center gap-3">
                              <span className="text-2xl">{category.category.icon}</span>
                              <div>
                                <h4 className="font-semibold">{category.category.name}</h4>
                                <p className="text-sm text-muted-foreground">{formatCount(category.transactions)} transactions</p>
                              </div>
                            </div>
                            <div className="text-right">
                              <p className="font-semibold">{formatNumber(category.revenue)}</p>
                              <p className="text-sm text-muted-foreground">{formatNumber(category.kickbackPaid)} kickback</p>
                            </div>
                          </div>
                        ))}
                      </div>
                    ...
                    <CardContent>
                      <div className="space-y-3">
                        {revenueAnalytics.topDeals.slice(0, 10).map((deal, index) => (
                          <div key={deal.deal.id} className="flex items-center justify-between p-3 rounded-lg bg-muted">
                            <div className="flex items-center gap-3">
                              <span className="text-sm font-bold text-muted-foreground">#{index + 1}</span>
                              <div>
                                <h4 className="font-semibold">{deal.deal.title}</h4>
                                <p className="text-sm text-muted-foreground">{deal.deal.category.name} • {deal.deal.dealType.name}</p>
                              </div>
                            </div>
                            <div className="text-right">
                              <p className="font-semibold">{formatNumber(deal.revenue)}</p>
                              <p className="text-sm text-muted-foreground">{formatCount(deal.uniqueCustomers)} customers</p>
                            </div>
                          </div>
                        ))}
                      </div>
