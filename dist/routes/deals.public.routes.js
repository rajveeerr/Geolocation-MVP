...
function formatDealForFrontend(deal, distance) {
    return {
        id: deal.id,
        title: deal.title || '',
...
function logQueryPerformance(operation, startTime, resultCount, filters) {
...
router.get('/deals', async (req, res) => {
    try {
        ...
        let deals = await prisma_1.default.deal.findMany({
            where: whereClause,
            include: {
                merchant: {
                    select: {
        ...
        if (deals.length) {
            deals = deals.filter(d => {
                if (d.dealType !== 'RECURRING')
                    return true;
                if (!d.recurringDays)
                    return false; // malformed recurring deal, hide
        ...
        if (latitude && longitude && radius) {
            ...
            deals = await prisma_1.default.deal.findMany({
                where: whereClause,
                include: {
                    merchant: {
                        select: {
            ...
            const dealsWithDistance = deals
                ...
                const distance = calculateDistance(userLat, userLon, deal.merchant.latitude, deal.merchant.longitude);
        ...
        if (Array.isArray(deals) && deals.length) {
            const isFormatted = !('merchant' in deals[0] && !deals[0].merchant.businessName); // naive check
            // deals are already formatted above (or include distance) at this point
            const happyHourDeals = deals.filter(d => d.dealType === 'HAPPY_HOUR');
            const otherDeals = deals.filter(d => d.dealType !== 'HAPPY_HOUR');
...
router.get('/deals/featured', async (req, res) => {
    try {
        ...
        let candidates = await prisma_1.default.deal.findMany({
            where: {
                startTime: { lte: now },
                endTime: { gte: now },
        ...
        scored.sort((a, b) => {
            ...
            if (b.typePriority !== a.typePriority)
